...
- [Link ref](https://github.com/josephmachado/data_engineering_best_practices/blob/main/adventureworks/pipelines/sales_mart.py)
...
import logging 
import os
from abc import ABC, abstractmethdod
...
from pyspark.sql import DateFrame, SparkSession
from pyspark.sql.functions import col, current_timestamp, expr, lit


try :
	from adventureworks.pipeline.utils.create_fake_data import generate_bronze_data
...
class StandardETL(ABC) : 
	...
	def run_data_validation(self, input_datasets: Dict[str, DeltaDataSet]) : 
		...
		validations = []
		for input in input_datasets.values() :
			validations.append(
				{
			...
			return context.run_checkpoint(
				checkpoint_name = "dp_checkpoint", validations = validations
			).list_validation_results()

	def validate_data(self, input_datasets : Dict[str,DeltaDataSet]) -> bool :
		results = {}
		for validation in self.run_data_validation(input_datasets) : 
			results[
		...
		for k,v in results.items():
			if not v :
				raise InValidDataException(
					f"The {k} dataset did not pass validation, please check " " the metadata fb for more information"
				)
		return True


	def check_required_inputs(
		self, input_datasets : Dict[str,DeltaDataSet], required_ids : List[str]
	) -> None : 
		if not all([ds in input_datasets for ds in required_ids]) : 
			raise ValueError(
				f"The input_datasets {input_datasets.keys()} dose not contain"
				f"{required_ds}"
			)
	def construct_json_string(self, keys: List[str]) -> str :
		...
		for input_dataset in input_datasets.values():
			if not input_dataset.skip_publish:
				curr_data = input_dataset.curr_data.withColumn(
					"elt_inserted", current_timestamp()
	...
	def run(self, spark : SparkSession, **kwargs) :
		...
		sefl.validate_data(silver_data_sets)
...
class SalesMartETL(StandardETL) :
	...
	def get_dim_customer(
		...
		customer_df_insert_net_new = (
			...
			.withColumn("valid_to",lit("2099-01-01 12:00:00.000"))
		)
		...
		return customer_df_inser_net_new.unionByName(
			customer_df_insert_existing_ids
		).unionByName(customer_df_ids_update)


	def get_fct_orders(
		self, input_datesets : Dict[str,DeltaDataSet],
		spark : SparkSession,
		**kwargs,
	) -> DataFrame :
	...
	def get_silver_datasets(
		...
		**kwargs ,
		...
		silver_datasets = {}
		silver_dataset["dim_customer"] = DeltaDataSet(
			name = "dim_customer",
			...
			data_type = "delta",
			databse  = f"{self.DATABASE}",
			partition = kwargs.get("partition", self.DEFAULT_PARTITION),
			
		)
		self.publist_data(silver_datasets, spark)
	...
	def get_sales_mart (
		self, input_dataset : Dict[str,DeltaDataSet],
		**kwargs
	) -> DataFrame : 
		dim_customer = (
		...
		fct_orders = input_dataset["fct_orders"].curr_data
		return (
			...
			.select(
				expr('to_date(fct_orders.delivered_on,"yyyy-dd-mm")').alias("deliver_date"),
